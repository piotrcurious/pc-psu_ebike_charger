...
int main() {
    ...
    for (int i = 0; i < 10000; ++i) {
        setAnalogRead(0, (int)(sim.batteryVoltage / VOLTAGE_SENSE_FACTOR));
        setAnalogRead(5, sim.getTempRaw(sim.temperature));
        bool psu_on = charger.isPsuOn();
        float current = (psu_on && sim.getBoostVoltage(charger.pwmDuty(), psu_on) > sim.batteryVoltage) ?
                        (sim.getBoostVoltage(charger.pwmDuty(), psu_on) - sim.batteryVoltage) / 0.6 : 0;
    ...
    else std::cout << "TEST FAILED: State=" << (int)charger.state() << std::endl;
    return 0;
}
